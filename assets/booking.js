/* =============================================================================
   ...
   • Offene Kurse (Worbis) werden von BWW veranstaltet. Deren `buchungs_url`
     enthält bereits `org=bww` und wird unverändert übernommen (der Veranstalter
     steckt in der URL; Umschreiben bricht die Zuordnung).
...
(function () {
  ...
  // Rechtssicherheit (§ 23 Abs. 2 SGB VII): BG/UK-Erstattung gilt NUR für Ersthelfer-Ausbildung,
  // Ersthelfer-Fortbildung und Erste Hilfe in Bildungs-/Betreuungseinrichtungen. Der geteilte Feed
  // liefert bg_uk_abrechenbar teils fälschlich true (Brandschutz, Betriebssani, Notfalltraining,
  // Lehrkräfte, Sanitätshelfer) → strengere Whitelist als zusätzliche Absicherung gegen eine
  // falsche Rechtsaussage im Live-Badge. Bleibt auch nach Feed-Korrektur richtig (strenger).
  ...
  function initTermine() {
    ...
    Array.prototype.forEach.call(els, function (el) {
      // Vorgerenderte Zeilen NICHT durch das Platzhalter-Gerippe ersetzen: sie stehen seit dem
      // Bau im HTML (static-first) und sind das, was Crawler/Antwortmaschinen sehen. Der
      // Erfolgsfall unten ersetzt sie gleich durch frische — frisch schlägt gebacken, aber
      // erst, wenn die frischen Daten wirklich da sind.
    ...
    loadFeed().then(function (all) {
      ...
      Array.prototype.forEach.call(els, function (el) {
        // Feed zur Laufzeit weg? Dann sind die vorgerenderten Zeilen das Beste, was wir haben —
        // höchstens einen Tag alt. Sie durch „lässt sich nicht laden" zu ersetzen wäre ein
        // Rückschritt gegenüber dem Zustand vor dem Vorrendern.
        if (el.getAttribute('data-prerendered')) return;
